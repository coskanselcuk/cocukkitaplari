...
import { Gamepad2, Trophy, Star, Lock, Puzzle, Grid3X3, HelpCircle, Palette } from 'lucide-react';
import { miniGames } from '../../data/mockData';

const GamesPage = () => {
  const gameCategories = [
    { name: 'Yapboz Oyunları', icon: Puzzle, games: miniGames.filter(g => g.type === 'puzzle'), color: 'from-purple-400 to-purple-600' },
    { name: 'Eşleştirme', icon: Grid3X3, games: miniGames.filter(g => g.type === 'matching'), color: 'from-green-400 to-green-600' },
    { name: 'Bilgi Yarışması', icon: HelpCircle, games: miniGames.filter(g => g.type === 'quiz'), color: 'from-blue-400 to-blue-600' },
    { name: 'Boyama', icon: Palette, games: miniGames.filter(g => g.type === 'coloring'), color: 'from-pink-400 to-pink-600' },
  ];
      ...
      <div className="px-4 mb-8">
        <div className="bg-gradient-to-r from-orange-400 to-pink-500 rounded-3xl p-6 shadow-2xl relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <h2 className="text-white text-2xl font-bold mb-2">Hafıza Oyunu</h2>
            <p className="text-white/80 mb-4">Kartları eşleştir ve hafızanı test et!</p>
            <button className="bg-white text-orange-500 font-bold py-3 px-6 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105">
              Oyna
            </button>
          </div>
        </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                {[...Array(4)].map((_, index) => {
                  ...
                  return (
                    <button
                      key={index}
                      className={`bg-white rounded-2xl p-4 shadow-lg relative overflow-hidden ${isLocked ? 'opacity-60' : 'hover:shadow-xl hover:scale-[1.02]'} transition-all duration-300`}
                      disabled={isLocked}
                    >
                      {isLocked && (
